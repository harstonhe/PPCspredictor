...
# --- Model Loading ---
# Construct a robust, absolute path to the model file.
# This ensures the app can find the model regardless of the execution directory.
try:
    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
except NameError:
    # In some Streamlit execution environments, __file__ is not defined.
    # In that case, we assume a relative path from the repo root.
...
with tab1:
    ...
    if calculate and model is not None:
        ...
        # The keys MUST EXACTLY match the feature names the model was trained on.
        raw_params = {
            'Pre-INR': pre_inr,
            'Pre-AST': pre_ast,
            'Operative time': op_time,
            'Post-R/F Ratio': post_rf,
        ...
        # This logic is based on the original HTML's JavaScript and is CRUCIAL.
        # The model was likely trained on normalized data, so we must apply the same transformation.
        normalized_params = {
            'Pre-INR': (raw_params['Pre-INR'] - 0.5) / 2.5,
            'Pre-AST': (raw_params['Pre-AST'] - 10) / 190,
            'Operative time': (raw_params['Operative time'] - 30) / 570,
            'Post-R/F Ratio': 1 - ((raw_params['Post-R/F Ratio'] - 100) / 400), # Inverted logic
            'Blood loss': raw_params['Blood loss'] / 2000,
            'Post-WBC': (raw_params['Post-WBC'] - 3) / 17,
            'Smoking': raw_params['Smoking']
        }
        
        # This feature order must also EXACTLY match the one used during training.
        # The order is now based on the UI element order, which is the most likely
        # to match the original dataframe's column order.
        feature_order = [
            'Pre-INR', 'Pre-AST', 'Operative time', 'Post-R/F Ratio',
            'Blood loss', 'Post-WBC', 'Smoking'
        ]
        
        # Create a DataFrame from the NORMALIZED data for the model
        input_df = pd.DataFrame([normalized_params])
